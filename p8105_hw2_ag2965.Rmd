...
```{r}
mrtrash = 
  readxl::read_excel("p8105_hw2_data/TrashWheel.xlsx", sheet="Mr. Trash Wheel", range= "A2:N550")%>%
  janitor::clean_names()%>%
  drop_na(dumpster)
```
...
```{r}
proftrash = 
  readxl::read_excel("p8105_hw2_data/TrashWheel.xlsx", sheet="Professor Trash Wheel", range= "A2:M97")%>%
  janitor::clean_names()%>%
  mutate(
    dumpster=as.numeric(dumpster))%>%
filter(dumpster>0)
